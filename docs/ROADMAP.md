...
Evidence-conditioned prompt: one template (`src/generation/prompt.py`),
dynamically conditioned by `src/generation/signals.py` on three signals
computed purely from the input chunks — no branch-specific separate
templates:
...
discipline as the Sprint 4 reranker-vs-reranker deferral above). Revisit
...
**Testing approach — two distinct kinds, reusable for future generation
work, not a one-off for this sprint** (`tests/test_generation.py`):
...
**Not yet done, tracked for a fast-follow:** the "preliminary end-to-end
evaluation (RAGAS)" named in this sprint's original scope line above was
not part of this pass — this pass covers the generation pipeline and
prompt itself plus Test A/B coverage only.
